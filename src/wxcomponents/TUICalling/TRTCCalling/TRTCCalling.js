...
class TRTCCalling {
	...
	initData() {
		const data = {
			...
			// ===================================
			timer2: null, // 我自定义的通话时长
			chatTimeNum2: 0, // 我自定义的聊天时长
	...
	handleNewInvitationReceived(event) {
		...
		if (callEnd) {
			...
			this.TRTCCallingDelegate.onCallEnd({
				userID: inviter,
				callEnd: isGroupCall ? 0 : inviteData.call_end
			});
			this._reset();
			return;
		}
	...
	handleInviteeRejected(event) {
		// console.log('结束拒绝的回调忙线结束拒绝的回调忙线结束拒绝的回调忙线',event)
		// console.log(
		...
		if (oldLineBusy || newLineBusy) {
			...
			CallLog({
				kf_user_id: event.data.invitee, // 客服id
				talk_type: 0, // 通话类型  0未接通  1已接通
				talk_time: 0, //通话时长
			}, function(res) {
				if (res.status == 1) {
	...
	handleSDKReady() {
		...
		promise
			.then((imResponse) => {
				...
				// console.log("获取拨打电话人资料",this.data.localUser)
			})
			.catch((imError) => {
	...
	async call(params) {
		...
		try {
			...
			// console.log(`数据数据${TAG_NAME} call(userID: ${userID}, type: ${type}) success, ${res}`);
			// 发起人进入calling状态
	...
	async hangup() {
		...
		if (this.data.isSponsor && this.data.callStatus === CALL_STATUS.CALLING) {
			...
			this.TRTCCallingDelegate.onCallEnd({
				message: cancelRes[0].data.message
			});
		}
		this.exitRoom();
		this._reset();
	...
	CallLogFun(kfId) {
		// console.log('客服id', kfId)
		if (kfId) {
			CallLog({
				kf_user_id: kfId, // 客服id
				talk_type: this.data.timer ? 1 : 0, // 通话类型  0未接通  1已接通
				talk_time: this.data.chatTimeNum2, //通话时长
			}, function(res) {
				if (res.status == 1) {
					console.log('返回的数据', res);
				}
	...
	_setCallStatus(status) {
		...
		if (this.data.callStatus == 'calling') {
			this.data.innerAudioContext.src = 'https://xcx.gdqhwy.com/public/static/mp3/laidian.mp3'
		...
		if (status == 'idle' && this.data.timer2 && this.data.chatTimeNum2 != 0) {
			this.CallLogFun(this.data.inviteeU)
		}
		...
		switch (status) {
			...
			case CALL_STATUS.IDLE:
				...
				this.data.chatTimeNum2 = 0;
				this.data.inviteeU = null;
				break;
		}
	}
