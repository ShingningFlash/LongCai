...
<uni-shadow-root class="TUICalling-component-calling-calling"><view class="invite-call" v-if="callType === 2">
    ...
    <view class="invite-calling" style="background-color: #667D8B;">
        <view class="invite-calling-header" v-if="remoteUsers.length === 1">
            ...
            <view class="invite-calling-header-right">
                ...
                <image :src="remoteUsers[0].avatar || '../../static/default_avatar.png'" @error="handleErrorImage"></image>
            </view>
        </view>
        <view class="invite-calling-header invite-calling-list" v-else>
             <view v-for="(item,index) in (remoteUsers)" :key="item.userID" class="invite-calling-item">
                 <image :src="item.avatar || '../../static/default_avatar.png'" @error="handleErrorImage"></image>
...
<view class="incoming-call audio-call" v-if="callType === 1" style="background-color: #667D8B;">
    <view class="invite-calling-single" v-if="remoteUsers.length === 1">
        <image :src="remoteUsers[0].avatar || '../../static/default_avatar.png'" @error="handleErrorImage"></image>
        <view class="tips" style="color: #fff;font-weight: 700;">{{remoteUsers[0].nick || remoteUsers[0].userID}}</view>
        <view v-if="isSponsor && callType === 1" class="tips-subtitle" style="color: #fff;">{{'等待对方接受'}}</view>
    </view>
    <view class="invite-calling-header invite-calling-list" v-else>
            <view v-for="(item,index) in (remoteUsers)" :key="item.userID" class="invite-calling-item">
                <image :src="item.avatar || '../../static/default_avatar.png'" @error="handleErrorImage"></image>
            <view class="invite-calling-item-message">
                <label class="tips" style="color: #fff;font-weight: 700;">{{item.nick || item.userID}}</label>
                <text class="tips-subtitle" v-if="(!isSponsor)">邀请你视频通话</text>
                <text class="tips-subtitle" v-else style="color: #fff;">等待对方接受</text> 
            </view>
        </view>
    </view>
    <view class="footer">
