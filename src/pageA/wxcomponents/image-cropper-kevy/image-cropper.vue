<template>
<uni-shadow-root class="image-cropper-kevy-image-cropper"><view class="image-cropper" @touchmove.stop.prevent="_preventTouchMove">
    <view class="main">
      <view class="content">
      ...
      <image v-if="imgInited" @load="imageLoad" :style="'width: '+(img_width ? img_width + 'px' : 'auto')+';height: '+(img_height ? img_height + 'px' : 'auto')+';transform: translate3d( '+(_img_left-img_width/2)+'px, '+(_img_top-img_height/2)+'px, 0) scale('+(scale)+') rotate('+(angle)+'deg);transition-duration:'+(0)+'s;display:'+(export_scale==5&&isSfzLoad || export_scale!=5?'inline-block':'none')" class="img" :src="imgSrc"></image>
      <view @touchstart="_start" @touchmove="_move" @touchend="_end" class="t-touch-area" :style="'width: '+(width)+'px; height: '+(height)+'px;left:'+(cut_left)+'px;top: '+(cut_top)+'px;'"></view>
      <image :src="xk_mb_url" v-if="xk_mb_url" class="t-touch-area" :style="'width: '+(width)+'px; height: '+(height)+'px;left:'+(cut_left)+'px;top: '+(cut_top)+'px;z-index:9;'"></image>
    </view>
...
<wxs module="m1" src="./image-cropper-m1.wxs"></wxs>
<script>

global['__wxRoute'] = 'image-cropper-kevy/image-cropper'
Component({
  properties: {
    /**     
     * 图片路径
...
export default global['__wxComponents']['image-cropper-kevy/image-cropper']
</script>
