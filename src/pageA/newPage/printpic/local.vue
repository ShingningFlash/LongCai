		...
		<!-- <view class="local-list" v-for="(item,index) in localList" :key="index">
				...
				<view class="right flex s-center">
					...
					<view class="btn" v-for="(items,indexs) in btns" :key="indexs">
						<view class="pre" :class="item.current == indexs ? 'activeBtn' : ''"
							@click="changeBtn(index,1,indexs)">
							{{items}}
						</view>
					</view>
			...
			<view class="" v-if="item.page > 1">
				<view class="inputs flex s-center m-end" v-if="item.current == 0">
					<input class="inputd" type="number" v-model="item.start">-
					<input class="inputd" type="number" v-model="item.end">
				</view>
				<view class="inputs flex s-center m-end" v-else>
					<input class="inputd" type="number" v-model="item.mid">
				</view>
			</view>
		...
		<view class="contents">
			<view class="images" style="position: relative;"  v-for="(item,index) in localList" :key="index">
				<image class="images" :src="'https://tm.ydlweb.com' +item.jobFile"
					...
					<image class="close"  src="/static/icons/close.png" @click="del(index)"></image>
			</view>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			console.log(uni.getStorage('files'));
		...
		methods: {
			...
			del(index) {
				...
				// uni.setStorageSync('filesLists',this.localList)
			},
			uploadFile(file) {
