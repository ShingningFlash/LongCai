	...
	<view>
		...
		<view class="orderTab">
			<scroll-view :scroll-x="true" :enable-flex='true'
				style="display: flex;justify-content: space-between;height: 70rpx;">
		...
		<view class="order-list-box">
			<view class="order-list-warp">
				<view class="order-list-item" v-for="(item,index) in OrderAll" :key='index'
					@click="clickJumpOrderDetail('/pages/orderDetail/orderDetail',item.order_id)">
					<view class="order-list-item-top">
						<view class="order-list-item-top-left">
							<text>订单编号：{{item.order_sn}}</text>
						</view>
						<view class="order-list-item-top-right">
					...
					<view class="totla-money-box">
						<view class="totla-money-warp">
							实付款：<text>￥{{item.total_amount}}</text>
						</view>
					</view>
					<view class="order-list-item-bottom">
						<view class="order-list-item-bottom-warp">
							<text class="text2" v-if="item.type == 4 || item.type == 5">再次购买</text>
							<text class="text2" v-if="item.type == 3" @click.stop="openPopupFun(item)">申请售后</text>
							<!-- <text class="text2" v-if="item.type == 3" @click.stop="clickJump('/pages/afterSalesOrder/afterSalesOrder')">申请售后</text> -->
							<!-- <text class="text2" v-if="item.type == 1 || item.type == 2"
								@click.stop="AllOrderOutFun(item.order_id)">取消订单</text> -->
							<text class="text1" v-if="item.type == 1"
								@click.stop="MiniPayOrderFun(item.order_id)">去支付</text>
							<text class="text1" v-if="item.type == 2 || item.type == 3"
								@click.stop="UserOrderAffirmFun(item.order_id)">确认收货</text>
							<text class="text1" v-if="item.type == 4" @click.stop="clickJumpEvaluation(item)">去评价</text>
						</view>
					</view>
				</view>
			</view>
		...
		<view class="popup" v-if="popupStatus">
			<view class="popupFlex">
				...
				<view class="model"
					@click="clickJump('/pages/returnGoods/returnGoods?order_id='+returnGoods.order_id+'&return_type=1')">
					<view class="left">
				...
				<view class="model"
					@click="clickJump('/pages/exchangeGoods/exchangeGoods?order_id='+returnGoods.order_id+'&return_type=2')">
					<view class="left">
	...
	export default {
		data() {
			return {
				...
				returnGoods: {}, // 申请退换货的订单信息
			}
		},
		onLoad(option) {
			that = this
			if (option.pay_type) {
				this.tabIndex = option.pay_type
			}
		},
		onShow() {
			this.UserOrderListFun()
		},
		methods: {
			// 打开弹出框
			openPopupFun(obj) {
			...
			// 跳转到订单详情页面
			clickJumpOrderDetail(e, orderid) {
				uni.navigateTo({
					url: e + '?order_id=' + orderid
				})
			},
			// 路由跳转
			clickJump(e) {
